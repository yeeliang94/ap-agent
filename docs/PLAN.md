...
- [x] 🟩 **R10: Verifying / Output / list touches + docs + verifier** — done 2026-08-19 (live verifier run pending an owner-triggered run: it needs the real model) —
  - *Two-axis code review (2026-08-19), fixed:* threshold `0` was read as "use the default" (Decimal falsy) — now only a missing value takes the default; `report_total` is the report's own figure only (an employee with no total cell reconciles as "no total to reconcile against", never against their own rows; the rows' sum rides on `summary.rows_total` for the screen); the formula pre-check no longer refuses a readable report — the read runs first and a *failed* read is explained by the uncomputed formulas; `REPORT_TOTAL_MISMATCH` honours the per-client switch; the correction-path re-check gets the worker's request cap; "same reading twice" compares structure only (not the AI's free-text `why`); receipts with no vendor are never "the same receipt"; `MISSING_REFERENCE` wording no longer embeds a code; the PRD names every catalogued code (a test greps it). *Kept as decisions:* a dated row with an amount is never a `skip_rows` entry (stricter than the plan's "no subtotal-like label"); a normal-vs-full-resolution km disagreement that the full-resolution read settles is a note, not an `EVIDENCE_UNCERTAIN` (the failure case is). — the rest of the surface, and proof.
