...
function AppShell() {
  ...
  return <><a className="skip-link" href="#main">Skip to content</a><header className="topnav"><Link className="brand" to="/invoices"><span className="logo">AP</span><span>AP Agent</span></Link><nav aria-label="Primary navigation"><Link to="/invoices" ariaCurrent={invoiceCurrent ? "page" : undefined}>Invoices</Link><Link to="/claims" ariaCurrent={claimCurrent ? "page" : undefined}>Claims</Link><Link to="/settings/workspace" ariaCurrent={settingsCurrent ? "page" : undefined}>Settings</Link></nav><GlobalRunIndicator /></header><main id="main" tabIndex={-1}>{route.page === "list" ? <RunListPage kind={route.kind} /> : route.page === "new" ? route.kind === "invoice" ? <InvoiceNewPage /> : <ClaimsNewPage /> : route.page === "run" ? <RunWorkspace kind={route.kind} runId={route.id} view={route.view} /> : route.page === "settings" ? <SettingsScreen section={route.section} /> : <section className="standard-page empty-state not-found"><span className="eyebrow">404</span><h1>Page not found</h1><p>Nothing lives at this address. The link may be out of date, or the run it pointed to may have been removed.</p><div className="actions center"><Link className="btn primary" to="/invoices">Invoice runs</Link><Link className="btn" to="/claims">Claim runs</Link></div></section>}</main></>;
}
