    ...
    <section>
      ...
      {runs && !polling && !error && (
        <p className="sub">
          ...
          <button className="btn sm" onClick={() => setRefresh((n) => n + 1)}>
            Refresh
          </button>
        </p>
      )}
      ...
      {runs && runs.length === 0 && (
        <div className="card">
          <div className="empty">
            <b>No claims runs yet</b>
            Start one above — upload the month's batch and the agent takes it from there.
          </div>
        </div>
      )}
      {runs?.map((r) => (
        <div key={r.id} className="card runcard">
          <div className="grow">
            <b>{r.client}</b>
            <span className="sub">
              {new Date(r.created_at).toLocaleString()} ·{" "}
              <span title={r.folder}>
                {r.folder.length > 48 ? "…" + r.folder.slice(-48) : r.folder}
              </span>
            </span>
            {/* A failed run's reason belongs on the row — it is the whole
                story of that run. */}
            {r.status === "failed" && r.error && <span className="sub error">{r.error}</span>}
          </div>
          <div className="rstats">
            <span>
              <b>{r.employee_count || "—"}</b>employees
            </span>
            <span>
              <b className={r.open_flags ? "bad" : ""}>
                {r.open_flags || (r.status === "ready" ? "0" : "—")}
              </b>
              open flags
            </span>
            {r.errors > 0 && r.status !== "failed" && (
              <span>
                <b className="bad">{r.errors}</b>error{r.errors === 1 ? "" : "s"}
              </span>
            )}
          </div>
